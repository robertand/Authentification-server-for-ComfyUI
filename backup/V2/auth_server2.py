...
DEFAULT_CONFIG = {
    "users": {
        "user1": {"password": "comfy.123", "comfy_url": "http://10.129.131.1:8184", "instances": 0, "max_instances": 2, "session_timeout": 60, "enabled": True},
        "user2": {"password": "comfy.123", "comfy_url": "http://10.129.131.2:8185", "instances": 0, "max_instances": 2, "session_timeout": 60, "enabled": True},
        "user3": {"password": "comfy.123", "comfy_url": "http://10.129.131.3:8186", "instances": 0, "max_instances": 2, "session_timeout": 60, "enabled": True},
        "user4": {"password": "comfy.123", "comfy_url": "http://10.129.131.4:8187", "instances": 0, "max_instances": 2, "session_timeout": 60, "enabled": True}
    },
    "admin": {
        "password": "admin123",
        "enabled": True
...
CLOUDFLARED_URLS = {}  # Cache pentru URL-urile cloudflared {local_url: cloudflared_url}

def ensure_cloudflared_installed():
    """Verifică și instalează cloudflared dacă nu există"""
    try:
        result = subprocess.run(['cloudflared', '--version'], 
                              capture_output=True, text=True, shell=False)
        if result.returncode == 0:
            log.info(f"✓ cloudflared is installed: {result.stdout.strip()}")
            return True
    except FileNotFoundError:
        log.warning("cloudflared not found. Will use local URLs only.")
        log.info("To install cloudflared:")
        log.info("Linux: wget https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64 -O /usr/local/bin/cloudflared && chmod +x /usr/local/bin/cloudflared")
        log.info("macOS: brew install cloudflared")
        return False

def start_cloudflared_tunnel(local_url, port):
    """Start cloudflared tunnel for local URL and return public URL"""
    try:
        log.info(f"Starting cloudflared tunnel for {local_url}")
        
        # Pornim procesul cloudflared
        ...
        # Așteptăm să obținem URL-ul
        public_url = None
        start_time = time.time()
        
        # Citim output-ul pentru a găsi URL-ul
        while time.time() - start_time < 15:  # Timeout de 15 secunde
            line = process.stderr.readline()
            if not line and process.poll() is not None:
                break
            
            # Căutăm URL-ul generat în output (formate diferite)
            patterns = [
                r'https://[a-zA-Z0-9\-]+\.trycloudflare\.com',
                r'https://[a-zA-Z0-9\-]+\.[a-zA-Z0-9\-]+\.cloudflarestorage\.com',
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    log.info(f"Cloudflared tunnel created: {public_url}")
                    break
        ...
        if public_url:
            # Salvăm procesul și URL-ul
            CLOUDFLARED_PROCESSES[port] = {
                'process': process,
                'url': public_url,
                'local_url': local_url,
                'start_time': time.time()
            ...
            # Salvăm în cache
            CLOUDFLARED_URLS[local_url] = public_url
            
            # Pornim un thread pentru a monitoriza procesul
            def monitor_process(proc, port_num):
                proc.wait()
                if port_num in CLOUDFLARED_PROCESSES:
                    del CLOUDFLARED_PROCESSES[port_num]
                    log.warning(f"Cloudflared tunnel for port {port_num} stopped")
            
            threading.Thread(target=monitor_process, args=(process, port), daemon=True).start()
            
            return public_url
        else:
            log.error(f"Could not get cloudflared URL for {local_url}")
            try:
                process.terminate()
        ...
        log.error(f"Error starting cloudflared for {local_url}: {e}")
        return None

def get_cloudflared_url(local_url):
    """Obține URL cloudflared pentru un URL local"""
    # Extrage portul
    ...
    # Verifică dacă avem deja un proces pentru acest port
    if port in CLOUDFLARED_PROCESSES:
        try:
            proc_data = CLOUDFLARED_PROCESSES[port]
            # Verifică dacă procesul încă rulează
            if proc_data['process'].poll() is None:
                return proc_data['url']
            else:
                # Procesul a murit, îl curățăm
                del CLOUDFLARED_PROCESSES[port]
                if local_url in CLOUDFLARED_URLS:
                    del CLOUDFLARED_URLS[local_url]
        except:
            pass
    
    # Verifică cache
    if local_url in CLOUDFLARED_URLS:
        return CLOUDFLARED_URLS[local_url]
    
    # Încearcă să pornim un nou tunnel
    cloudflared_installed = ensure_cloudflared_installed()
    if not cloudflared_installed:
        return None
    
    return start_cloudflared_tunnel(local_url, port)

def get_optimized_url(local_url):
    """Obține URL-ul optimizat (cloudflared pentru IP-uri 10.129.131.*)"""
    # Verifică dacă este adresa locală 10.129.131.*
    if re.match(r'http://10\.129\.131\.\d+:\d+', local_url):
        log.info(f"Detected local network URL: {local_url}, trying cloudflared...")
        cloudflared_url = get_cloudflared_url(local_url)
        if cloudflared_url:
            log.info(f"Using cloudflared tunnel: {cloudflared_url}")
            return cloudflared_url
    
    # Dacă cloudflared eșuează sau URL-ul nu e 10.129.131.*, folosește cel original
    return local_url

def cleanup_cloudflared_processes():
    """Curăță procesele cloudflared vechi"""
    current_time = time.time()
    expired_ports = []
    
    for port, data in CLOUDFLARED_PROCESSES.items():
        # Închide procesele mai vechi de 2 ore sau dacă s-au închis
        if current_time - data['start_time'] > 7200 or data['process'].poll() is not None:
            try:
    ...
    for port in expired_ports:
        local_url = CLOUDFLARED_PROCESSES[port]['local_url']
        if local_url in CLOUDFLARED_URLS:
            del CLOUDFLARED_URLS[local_url]
        del CLOUDFLARED_PROCESSES[port]
        log.info(f"Cleaned up cloudflared tunnel for port {port}")
...
<style>
    body {font-family: 'Segoe UI', Arial, sans-serif; text-align: center; margin-top: 100px; background: #1a1a1a; color: #e0e0e0;}
    .container {width: 500px; margin: 0 auto; background: #2a2a2a; padding: 30px; border-radius: 10px; box-shadow: 0 0 20px rgba(0,0,0,0.5);}
    .warning {color: #ffc107; font-size: 18px; margin-bottom: 20px;}
...
def check_single_instance(comfy_url):
    """Verifică o singură instanță ComfyUI, încercând și cloudflared pentru IP-uri 10.129.131.*"""
    global comfy_instances_ready
    
    # Verifică dacă URL-ul este 10.129.131.*
    is_10_network = re.match(r'http://10\.129\.131\.\d+:\d+', comfy_url)
    
    if is_10_network:
        # Încearcă să obții URL cloudflared
        cloudflared_url = get_cloudflared_url(comfy_url)
        urls_to_try = [cloudflared_url, comfy_url] if cloudflared_url else [comfy_url]
        log.info(f"Checking 10.129.131.* instance {comfy_url}, trying {len(urls_to_try)} URL(s)")
    else:
        urls_to_try = [comfy_url]
        log.info(f"Checking non-10.129.131.* instance {comfy_url}")
    
    for url in urls_to_try:
        if not url:
            continue
            
        for i in range(20):  # 20 de încercări (20 secunde)
            try:
                r = requests.get(f"{url}/", timeout=2)
                if r.status_code == 200:
                    comfy_instances_ready[comfy_url] = True
                    if url != comfy_url:
                        log.info(f"ComfyUI instance {comfy_url} is accessible via cloudflared: {url}")
                    else:
                        log.info(f"ComfyUI instance {comfy_url} is accessible directly")
                    return True
            except Exception as e:
                if i % 5 == 0:
                    log.info(f"Waiting for ComfyUI {comfy_url} via {url}... ({i}/20)")
            time.sleep(1)
    
    log.warning(f"ComfyUI instance {comfy_url} failed to start within timeout")
    return False
...
class ComfyUIIframeHandler(BaseHandler):
    def get(self):
        ...
        # Obține URL-ul optimizat (cloudflared dacă e 10.129.131.*)
        optimized_url = get_optimized_url(comfy_url)
        ...
        <head>
            ...
            <style>
                ...
                .auth-header {{
                    ...
                    padding: 0 10px; /* MODIFICAT: padding redus */
                    display: flex;
                    justify-content: space-between;
                    align-items: center;
                    z-index: 10000;
                    ...
                    height: 25px; /* MODIFICARE: jumătate din înălțime */
                }}
                .server-title {{
                    position: absolute;
                    left: 50%;
                    ...
                    color: white;
                    font-size: 14px; /* MODIFICAT: font mai mic */
                    font-weight: bold;
                    text-align: center;
                }}
                .user-info {{
                    color: #007bff;
                    font-weight: bold;
                    font-size: 12px; /* MODIFICAT: font mai mic */
                }}
                .auth-buttons {{
                    display: flex;
                    gap: 5px; /* MODIFICAT: gap redus */
                }}
                .auth-btn {{
                    background: #007bff;
                    color: white;
                    border: none;
                    padding: 2px 6px; /* MODIFICAT: padding redus semnificativ */
                    border-radius: 2px; /* MODIFICAT: border-radius redus */
                    cursor: pointer;
                    font-size: 10px; /* MODIFICAT: font mult mai mic */
                    text-decoration: none;
                    display: inline-flex;
                    align-items: center;
                    justify-content: center;
                    height: 18px; /* MODIFICAT: înălțime redusă */
                    min-width: 25px; /* MODIFICAT: lățime minimă redusă */
                }}
                .auth-btn.logout {{
                    background: #dc3545;
                }}
                ...
                #comfyIframe {{
                    position: fixed;
                    top: 25px; /* MODIFICARE: ajustat pentru bara mai mică */
                    left: 0;
                    width: 100%;
        ...
        <body>
            <div class="auth-header">
                ...
                <div class="server-title">PRO AI Server</div> <!-- MODIFICARE: text centrat -->
                <div class="auth-buttons">
                    <button class="auth-btn" onclick="openAboutModal()">?</button>
                    <button class="auth-btn settings" onclick="openUserSettingsModal()">SET</button>
            ...
            <button class="workflow-save-btn" onclick="saveCurrentWorkflow()">SAVW</button>
            <button class="workflow-btn" onclick="openWorkflowBrowser()">LODW</button>
            <button class="chat-btn" onclick="toggleChatModal()" id="chatButton">
                MSG
                <div class="chat-notification" id="chatNotification" style="display:none">!</div>
            </button>
            
            <iframe id="comfyIframe" src="{optimized_url}?auth_token={token}&embedded=true"></iframe>
    ...
    def post(self):
        ...
        if username:
            ...
            USERS[username] = {
                ...
                "comfy_url": data.get("comfy_url", f"http://10.129.131.9:8189"),
                "instances": 0,
...
class MultiInstanceProxyHandler(BaseHandler):
    ...
    def get_user_comfy_url(self):
        ...
        original_url = session_data.get("comfy_url")
        
        # Obține URL-ul optimizat (cu cloudflared dacă e 10.129.131.*)
        optimized_url = get_optimized_url(original_url)
        
        # Salvează URL-ul optimizat în sesiune pentru reutilizare
        if optimized_url != original_url:
            session_data["optimized_url"] = optimized_url
...
class MultiInstanceWebSocketProxy(tornado.websocket.WebSocketHandler):
    ...
    def get_user_comfy_ws_url(self):
        ...
        original_url = session_data.get("comfy_url", "http://10.129.131.1:8189")
        
        # Obține URL-ul optimizat (folosește cel salvat dacă există)
        optimized_url = session_data.get("optimized_url")
        if not optimized_url:
            optimized_url = get_optimized_url(original_url)
        
        # Convertim HTTP/HTTPS la WebSocket
        if optimized_url.startswith("https://"):
            ws_url = optimized_url.replace("https://", "wss://") + "/ws"
        elif optimized_url.startswith("http://"):
            ws_url = optimized_url.replace("http://", "ws://") + "/ws"
        else:
            ws_url = optimized_url + "/ws"
        
        # Încercăm să ne conectăm prin cloudflared dacă URL-ul original e 10.129.131.*
        if re.match(r'http://10\.129\.131\.\d+:\d+', original_url):
            log.info(f"Attempting WebSocket via cloudflared for {original_url}")
...
class SimpleComfyProxyHandler(BaseHandler):
    async def get(self, path=None):
        ...
        target_url = f"{optimized_url}/{path}" if path else optimized_url
        if self.request.query:
            target_url += "?" + self.request.query
...
def periodic_cleanup():
    """Funcție periodică pentru curățare"""
    while True:
...
if __name__ == "__main__":
    ...
    print("Cloudflared Integration: Automatic for 10.129.131.* addresses")
    print("=================================================")
    ...
    try:
        ...
        print("Cloudflared integration: ACTIVE for 10.129.131.* addresses")
